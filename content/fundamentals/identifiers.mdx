...
<Callout type="info" emoji="ℹ️">
  We use the [Key Event Receipt
  Infrastructure](https://trustoverip.github.io/tswg-keri-specification/) (KERI)
  standard for managing identifiers and keys.
</Callout>
...
For now, individual identifiers are single-signature identifiers which means there is one key pair to sign statements, and one key pair to rotate keys.
...
<Callout type="info" emoji="ℹ️">
  Multi-signature identifiers in theory can be customised much further.
...
### Rotate Key of a Single Identifier
...
Delegation in KERI is known as co-operative delegation and is used in enterprise to help in top-down hierarchical recovery.
In a nutshell, a delegator must give a signed approval of the creation of a delegatee identifier and any key rotations.
The delegator may also be changed.
...
Delegated identifiers aren't supported in the wallet yet, but is indeed planned!

## Multi-Signature Identifiers with Threshold

Group identifiers are multi-signature by design. Each member of the group controls one of the signing keys and one of the rotation keys. A threshold can be set to decide how many members of a group are required to sign or rotate.

### Create a Multi-Signature Identifier with Threshold
<video src="/videos/CreateGroupIdentifier.mov" type="video/mp4" controls />
