...
There are various implementations of KERI out there with a reference implementation called [keripy](https://github.com/WebOfTrust/keripy).
keripy implements KERI, ACDC and CESR at a low level, along with some helper functionality and a cli tool called the kli.
One could take keripy as is an integrate it into any Python backend directly, but we don't recommend this unless you are familiar with the implementation.
...
It also means that you can deploy a zero trust network architecture across your services in a more lighweight and approachable way.
...
- **3902:** This port must be open to every other KERI agent you want to interact with, so this is typically publically accessible.
...
This seed is used to generate public-private key pairs deterministically, and the keys are used to secure the communication with KERIA and encrypt any crytographic key material we may want to store on KERIA.
