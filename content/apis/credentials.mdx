...
As mention before [TODO link], an ACDC can be more than just a credential as the issuee field is optional.
The Signify API was originally built with credential use cases in mind, so it's due a naming update as it supports untargeted ACDCs as well!
...
Technically, the issuance and revocation events in the registry form their own hashed-linked data structure called a Transaction Event Log, or TEL. As expected, TEL events are not signed directly, but rather are anchored in the KEL of the registry controller.
...
For now, schemas can be hand-crafted and SAIDified using this [tool](https://github.com/WebOfTrust/vLEI/tree/main).

Once a schema has been created, the `$id` (SAID) field of the JSON schema can be referenced when issing credentials.
Since we always reference using the SAID, there are no security concerns in schemas being updated by attackers.

Lets issue a credential to our new registry!
...
<Callout type="info">
  ...
  Sections in ACDCs can be "collapsed" using their SAID for compactness, which enables particual disclosure.
...
For many credentials, they will contain sensisitve private information and require further protection from a privacy point of view.
Adding additional `u` fields alongside any calculated SAIDs, such as the top-level credential SAID and the attributes block SAID provides sufficiently entrophy to protect from rainbow or dictionary attacks.
We can use the `Salter` class in Signify to produce a random salt to achieve this entrophy.
...
The true power of ACDCs is the ability to chain them together to form a property graph.
For each issued ACDC in the property graph, it defines its edges to existing ACDC have have been issued, and may in turn be linked to by other new ACDCs after.
...
The LE schema defines what edges are possible, and there can be multiple, and have more complex relationships using boolean logic like `AND` and `OR`, etc.
Edge rules such as `I2I` enforce that the issuer of this ACDC must be the issuee of the linked ACDC.
Edges can also have weights, and what the weights mean in ecosystem dependent.
...
<Callout type="info">
  ...
  For example, if a company wants to issue their quarterly reports as an ACDC, they can make the ACDC itself available however they want, such as on a web API, instead of disclosing to every verifier individually - that would be overkill!
...
On request, the discloser can disclose only the necessary amount of information within an ACDC necessary to further a business transaction.
The disclosee can then decide if they require more information to further the transaction again, and request more.
...
<Callout type="warning">
  To our knowledge, graduated disclosure still requires more development in keripy and is on our roadmap.
</Callout>

### Issuance flow

The full set of IPEX messages can be used to negotiate the terms of an issuance.
For now, we will focus on the more simple scenario where no negitation is required.
