...
The `multisig-incept.test.ts` integration test can be used to reference.
...
In the Signify APIs, and certainly if you ever contribute back to keripy ot KERIA, you will notice `mhab` and `ghab`.
A habitat, or hab is the term in keripy used to describe an identifier at the data layer.
Hence, `mhab` is member hab, and `ghab` is group hab.
...
Since it's a digest, a different initial digest, or a different ordering of the keys results in a different identifier.
...
A thresholf of `["2/3", "1/3", "1/3"]` requires signatures from Alice and Bob, or Alice and Carol.
...
<Callout type="warning">
  ...
  Please see [this discussion](https://github.com/WebOfTrust/keripy/discussions/934) related to group coordination; right now we recommend that the group leader always be the first to sign any message.
</Callout>
...
For a single-sig identifier, setting up the OOBI was pretty straight forward.
We simply authorized our KERIA agent to have the `agent` end role, which under the hood is signing an end role authorization object.
...
KEL-backed will either involve directly calling `identifiers.interact()` or indirectly by creating a registry, or issuing a credential, etc.
