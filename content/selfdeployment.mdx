...
We support two container-based methods in order to self-host the services needed to back the Veridian wallet:
...
1. **Download AND review the script**:
   ...
   ```
  **Important**: Before executing the script, review it and adapt the Helm installation command. The script contains a command that references a local chart directory, but you need to change it to reference the remote chart repository:
  ```sh
  helm install \
     ${HELM_RELEASE_NAME} \
