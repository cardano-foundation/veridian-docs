...
Lets imagine Alice and Bob both have an identity wallet installed on their mobile, and each is connected to a different KERIA service provider.
...
KERI identifiers (at least, non ephemeral ones) are a CESR hash (SAID) of the first event in the key event log, the inception event, and could look something like `ED4KeyyTKFj-72B008OTGgDCrFo6y7B2B73kfyzu5Int`.
...
When the endpoint is called (with a GET request), some crytographic material is returned, such as the KEL of the identifier.
This allows another party to first verify that the hash of the inception event equals the identifier in the URL, and that any other events in the KEL verify based on KERI rules.
...
An identifier created must authorize another identifier to act on this role in their behalf.
...
await client
  .identifiers()
  .addEndRole(pre, "agent", client.agent!.pre);
```

The above snippet will sign an end role authorization for your KERIA agent identifier (`client.agent!.pre`), and it is signed using your identifier (`pre`).
